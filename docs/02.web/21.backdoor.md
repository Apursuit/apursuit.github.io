...
在ctf web方向中，eval语言构造器不能直接执行linux系统命令例如`ls /`,`cat /flag`等，需要使用可以执行系统命令的函数，例如system,passthru,shell_exec等等或者可以用用反引号`包裹系统命令来执行，不会返回执行结果
