...
在收藏的一个网站(输入b站视频链接提取视频)中，抓包发现似乎存在很多验证内容，使用简单的爬虫发包不能拿到视频文件。那就想使用selenium库控制浏览器模拟真实用户

![https://the0n3.top/medias/video-selenium/data.png]
...
![https://the0n3.top/medias/video-selenium/version.png]
...
![https://the0n3.top/medias/video-selenium/path.png]
...
![https://the0n3.top/medias/video-selenium/page.png]
...
element.send_keys("https://www.bilibili.com/vidoe/xxxxxx")
time.sleep(2)
# 关闭浏览器
driver.quit()
```
...
element_tag.send_keys("https://www.bilibili.com/video/xxxx")
time.sleep(2)
# 只要第一个按钮
...
# 通过测试发现只有1个input
element_tag = driver.find_element(By.TAG_NAME,"input")
element_tag.send_keys("https://www.bilibili.com/video/BV1G74y1m76P")
time.sleep(2)
# 只要第一个按钮
button_tag = driver.find_element(By.TAG_NAME,"button")
button_tag.click()
...
到此，已经实现了输入url自动获取视频信息，如视频标题，视频url，把视频标题作为文件名，通过requests库来下载视频