...
看了师傅们解题思路，眼前一亮。猜测flag在data字段里，使用`delete from flag where data like'f%' and sleep(3)`，这里非常细节，还用到了and的性质，前假会直接结束，不会执行后面的sleep函数，只有正则匹配到flag时，才会执行sleep函数，从而盲注出flag
